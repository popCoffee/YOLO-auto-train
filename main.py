...
#####  
    ...
    def __init__(self, master):
        ...
        #xscrollbar.config(command=self.mainPanel.xview)
        #yscrollbar.config(command=self.mainPanel.yview)
...
##        self.loadDir()
    ...
    def loadDir(self, dbg = False):
        if not dbg:
            ...
            s = '/home/onr/src/HowToTrainYoloExample/BBox-Label-Tool-Multi-Class/Images/001'
#        if not os.path.isdir(s):
#            tkMessageBox.showerror("Error!", message = "The specified dir doesn't exist!")
#            return
        # get image list----
        self.imageDir = os.path.join(r'./Images', '%03d' %(self.category))
        #convert to jpg for viewing
        dir2 = os.path.join(r'Images', '%03d' %(self.category))
	if(OSs == 'Linux'):
			subprocess_cmd( ' mkdir CC1; cp ./%s/*.png ./CC1 '  % dir2)
			subprocess_cmd( '  cd CC1;for i in *.png; do convert "$i" "${i%.*}.jpg"; done;  '  )
			subprocess_cmd( '  cp ./CC1/*.jpg ./%s/ ; rm -r CC1'  % dir2)
			subprocess_cmd(' find ./%s/ -type f -name "*.jpeg" -print0 | xargs -0 rename "s/.jpeg$/.jpg/" ' % dir2 )
			subprocess_cmd(' find ./%s/ -type f -name "*.JPEG" -print0 | xargs -0 rename "s/.JPEG$/.jpg/" ' % dir2 )
			subprocess_cmd(' find ./%s/ -type f -name "*.JPG" -print0 | xargs -0 rename "s/.JPG$/.jpg/" ' % dir2 )
        #print self.imageDir 
        #print self.category        
...
#
#mousewheel_support = image_scroll(root)
#mousewheel_support.add_support_to( , xscrollbar=xscrollbar, yscrollbar=yscrollbar, what="pages")

if __name__ == '__main__':
    #mousewheel_support = Image_Support(root)
    root = Tk()
    tool = LabelTool(root)   
