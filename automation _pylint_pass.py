...
def test_train():
    ...
    # Current Directory
    directory_current = "\
    ".join([os.path.dirname(os.path.abspath("ALLrecentJpgTxt")), "/ALLrecentJpgTxt"])
...
if __name__ == "__main__":
    ...
    subprocess_cmd(' \
    cd Newimages; for i in *.png; do convert "$i" "Converted_img/${i%.*}.jpg"; done')
    subprocess_cmd(' cd Newimages ; for x in *.JPG; do mv "$x" "${x%.JPG}.jpg"; done  ')
    subprocess_cmd(' \
    cd Newimages ; for x in *.JPEG; do mv "$x" "${x%.JPEG}.jpg"; done  ')
    subprocess_cmd('\
    mkdir ALLrecentJpgTxt; cp -r %sNewimages/Converted_img/* %sALLrecentJpgTxt;\
     cp -r %sNewimages/*.jpg %sALLrecentJpgTxt ' % (PATH2B, PATH2B, PATH2B, PATH2B))
    ...
    ##MYPATH = "/home/user/src/
    #HowToTrainYoloExample/BBox-Label-Tool-Multi-Class/Labels/%s/" % str(FOLDERR)
    ##OUTPATH = "/home/user/
    #src/HowToTrainYoloExample/BBox-Label-Tool-Multi-Class/Labels/converted/"
    OUTPATH = "%s/converted/" % DIR_LABELS
    print MYPATH + '\n'

    #The class. cls = "wave"
    if CLS not in CLS_EACH:
        exit(0)
    IDENTITY1 = CLS_EACH.index(CLS)
    ...
    for (_, _, filenames) in walk(MYPATH):
        TXT_NAME_LS.extend(filenames)
        break
    print TXT_NAME_LS

    # Go through list
    for txt_name in TXT_NAME_LS:
        ...
        for datai in f_lines:
            ...
            if len(datai) >= 2:
                ...
                for _ in range(0, const):
                    ...
                    photo_path = str(' \
                    %s/ALLrecentJpgTxt/%s.jpg'%(PATHNAME, os.path.splitext(txt_name)[0]))
                    ...
                    txt_outfile.write(str(IDENTITY1) + " \
                    " + " ".join([str(a) for a in bag33]) + '\n')
                    cou = cou+1
    ...
    subprocess_cmd(' \
    cp -r %sLabels/converted/* %sALLrecentJpgTxt ; rm -r converted ' % (PATH2B, PATH2B))
